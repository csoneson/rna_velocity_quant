...
## Convert spliced and unspliced matrices to dense format. 
adata.layers['spliced'] = adata.layers['spliced'].todense()
adata.layers['unspliced'] = adata.layers['unspliced'].todense()

# Repeatedly perturb the spliced and unspliced normalized counts for each cell and gene and estimate velocities
random.seed(30)
outfile = open(plotdir + "/" + base + basegs + "/velocity_perturb.out", "w")
alpha = 0.5  ## scaling factor for the standard deviation in the perturbation
for i in range(15):
	try:
		adata2 = adata.copy()
		for j in range(adata2.shape[0]):
			for k in range(adata2.shape[1]):
				msp = adata2.layers['spliced'][j, k]
				msu = adata2.layers['unspliced'][j, k]
				adata2.layers['spliced'][j, k] = max(0, np.random.normal(loc = msp, scale = alpha * math.sqrt(msp + 0.5), size = 1))
				adata2.layers['unspliced'][j, k] = max(0, np.random.normal(loc = msu, scale = alpha * math.sqrt(msu + 0.5), size = 1))
